...
m60 <- lmer(DV2 ~
              Months.To.Election * exec_final_round
            + Amt.Out
            # + inv_grade
            + cr
            # # global liquidity is tight
...
names(sm)[2] <- 'Executive Vote Share (Only Round)'
...
ggplot(data = sm) + 
  theme_bw() +
  geom_line(aes(x = `Executive Vote Share (Only Round)`, y = AME), colour = new_colors[1]) +
  geom_ribbon(aes(x = `Executive Vote Share (Only Round)`, y = AME,
  ...
  xlab("Executive Vote Share (Only Round)") +
