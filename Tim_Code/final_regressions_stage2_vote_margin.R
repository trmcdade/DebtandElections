...
# dim(data_24[data_24$polity2 >= 5,])
# dim(data_24[data_24$polity2 >= 6,])
# dim(data_24[data_24$polity2 >= 4.5,])
...
sort(unique(data_120$Country))
unique(data_120$Country)
...
m24 <- lmer(DV2 ~
           ...
           + (1 | Country)
           + (1 | Curr)
           , data = data_24[(data_24[,'vote_margin'] != -999),]
)
summary(m24)
r.squaredGLMM(m24)
...
m60 <- lmer(DV2 ~
            ...
            + (1 | Country)
            + (1 | Curr)
            , data = data_60[(data_60[,'vote_margin'] != -999),]
)
summary(m60)
r.squaredGLMM(m60)
...
m120 <- lmer(DV2 ~
            ...
            + (1 | Country)
            + (1 | Curr)
            , data = data_120[(data_120[,'vote_margin'] != -999),]
)
summary(m120)
r.squaredGLMM(m120)
...
# keepcols <- c('Months.Mat.To.Election', 'vote_margin', 'Months.Mat.To.Election:vote_margin')
results_df
...
m <- margins(m24_usd, at = 
               list(vote_margin =
                      fivenum(data_24[(data_24$vote_margin != -999), 'vote_margin'], 
                              na.rm = TRUE)))
sm <- summary(m)
