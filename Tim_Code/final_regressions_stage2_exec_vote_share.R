...
# hist(data_120[, 'exec_vote_share'])
# hist(data_120[(data_120[,'exec_vote_share'] < 50), 'exec_vote_share'])
...
m24 <- lmer(DV2 ~
             Months.Mat.To.Election * exec_vote_share
           ...
           + (1 | Country)
           + (1 | Curr)
           , data = data_24[(data_24[,'exec_vote_share'] != -999),]
)
summary(m24)
r.squaredGLMM(m24)
...
m60 <- lmer(DV2 ~
            ...
            + (1 | Country)
            + (1 | Curr)
            , data = data_60[(data_60[,'exec_vote_share'] != -999),]
)
summary(m60)
r.squaredGLMM(m60)
...
m120 <- lmer(DV2 ~
            ...
            + (1 | Country)
            + (1 | Curr)
            , data = data_120[(data_120[,'exec_vote_share'] != -999),]
)
summary(m120)
r.squaredGLMM(m120)
...
# keepcols <- c('Months.Mat.To.Election', 'exec_vote_share', 'Months.Mat.To.Election:exec_vote_share')
results_df
...
# m <- margins(m24_usd, at = list(exec_vote_share = fivenum(data_24$exec_vote_share, na.rm = TRUE)))
m <- margins(m24_usd, at = 
               list(exec_vote_share =
                      fivenum(data_24[(data_24$exec_vote_share != -999), 'exec_vote_share'], 
                              na.rm = TRUE)))
sm <- summary(m)
...
ggplot(data = sm) + 
  theme_bw() +
  geom_line(aes(x = exec_vote_share, y = AME), colour = new_colors[1]) +
  geom_ribbon(aes(x = exec_vote_share, y = AME,
