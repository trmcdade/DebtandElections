...
m24 <- lmer(DV2 ~
              Months.To.Election * exec_vote_share
            + Amt.Out
            # + inv_grade
            + cr
...
names(sm)[2] <- 'Executive Vote Share (First or Only Round)'
...
ggplot(data = sm) + 
  theme_bw() +
  geom_line(aes(x = `Executive Vote Share (First or Only Round)`, y = AME), colour = new_colors[1]) +
  geom_ribbon(aes(x = `Executive Vote Share (First or Only Round)`, y = AME,
...
ggsave(paste(dir, glue('/ame_24m_usd_{var}.png'), sep = ''))

##############################
###### regression table ######
##############################

omit_vars <- c('Amt.Out', 'Amt.Out_USD', 'cr', 'US_FFR')
class(m24) <- "lmerMod"
