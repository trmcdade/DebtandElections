...
class Debt_Issues:
    ...
    def __init__(self, pull_date = None, sum_across_currencies = None, max_mty = None):
        ...
        self.s = self.s[['Country (Full Name)', 'Issue Date', 'Maturity Date', 'Curr', 'Next Election After Issuance', 'Months.To.Election', 'Amount Issued']].copy()
        ...
        self.s = self.s.groupby(['Country (Full Name)', 'Issue Date', 'Maturity Date', 'Curr', 'Next Election After Issuance', 'Months.To.Election']).agg(['sum'], dropna=False).reset_index()
    ...
    def sum_data_across_currencies(self, df):
        ...
        df['DV_USD'] = df['Amt.Mat_USD'].divide(df['Amt.Out_USD'], fill_value = 0)
    ...
    def sb_join(self):
        ...
        existing_file_list = [x for x in os.listdir(self.dir + '/Tim_Code/Output_Data') if x.startswith('share_of_outstanding_') and self.pull_date in x and f'{self.max_mty}m_' in x and self.sum_across_currencies in x]
        if len(existing_file_list) > 0:
            ...
            def output_outstanding(ctry, curr, date):
                """
                This function gets the total amount of outstanding debt in a given month for a country-currency combo.
                This will serve as the denominator.
                """
                amt_outstanding = si[(si['Country (Full Name)'] == ctry) & (si['Curr'] == curr) & (si['Issue Date'] <= date) & (si['Maturity Date'] >= date)]['Amount Issued'].sum()
                return amt_outstanding

            amt_outstanding_list = Parallel(n_jobs=int(os.cpu_count()))(delayed(output_outstanding)(si['Country (Full Name)'].iloc[i], si['Curr'].iloc[i], si['Issue Date'].iloc[i]) for i in tqdm(range(si.shape[0])))
            si['Amount Outstanding'] = amt_outstanding_list

            # write numerator: all debt outstanding today that matures today
            def matures_now(ctry, curr, date):
                """
                ...
                """
                amt_maturing = si[(si['Country (Full Name)'] == ctry) & (si['Curr'] == curr) & (si['Maturity Date'] == date)]['Amount Issued'].sum()
                return amt_maturing

            amt_maturing_list = Parallel(n_jobs=int(os.cpu_count()))(delayed(matures_now)(si['Country (Full Name)'].iloc[i], si['Curr'].iloc[i], si['Issue Date'].iloc[i]) for i in tqdm(range(si.shape[0])))
            si['Amount Maturing'] = amt_maturing_list
            ...
            sboutname = self.dir + f'/Tim_Code/Output_Data/share_of_outstanding_{self.sum_across_currencies}_{self.max_mty}m_{self.pull_date}.csv'
    ...
    def filter_data(self):
        ...
        final_cols = ['Country'
                        ...
                        ,'Month', 'month'
                        ,'Curr'
                        ,'Amt.Issued', 'Amt.Mat', 'Amt.Out'
                        ,'Amt.Issued_USD', 'Amt.Mat_USD', 'Amt.Out_USD'
        ...
        # write it out
        outname = self.dir + f'/Tim_Code/Output_Data/outstanding_regdata_{self.sum_across_currencies}_{self.max_mty}m_{self.pull_date}.csv'
